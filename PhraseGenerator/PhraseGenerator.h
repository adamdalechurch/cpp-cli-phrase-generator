 ...
 */
class PhraseGenerater {
public:
    bool isValid = false;

    PhraseGenerater(TwoDVector items, std::string seperator) {
        SetItems(items);
        SetSperator(seperator);
        CalcPermutations();
    ...
    void SetItems(TwoDVector items) {
        _items = items;
    }

    void SetSperator(std::string seperator) {
    ...
    std::vector<std::string> GetOutput() {
        ...
        for (int x = 0; x < _output.size(); x++) {
            for (int y = 0; y < _output[x].size(); y++) {
                res.push_back(_output[x][y] + _seperator);
            }
